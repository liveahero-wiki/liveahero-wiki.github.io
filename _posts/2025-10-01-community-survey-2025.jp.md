...
## プレイヤーが気に入っている点
  ...
  - ストーリー
    ...
    - キャラクター間の交流
    - ある回答者は、『放サモ』よりも性的暗示が少ないストーリーを持つ『ライブ・ア・ヒーロー！』を好む。
      - <span class="comment">「改善」セクションでは、他のユーザーがストーリーの性的暗示が不十分だと不満を述べているのが確認できます</span>
    - BGMが良い
...
### ストーリー
  ...
  - 一部の回答者は、『ライブ・ア・ヒーロー！』のストーリーが『東京放課後サモナー』と比べて性的な暗示がかなり少ないと不満を述べた。
  - デートクエストの実装
...
### ガチャと収益化
  ...
  - 一部の回答者から、期間限定のジェムパックではなく、月額パスの実装を求める声があった
      - 多くの人が、毎月より低い価格帯でゲームをサポートしたいと考えている
      - <span class="comment">これは、平均給与が日本よりはるかに低い一部の発展途上国では特に当てはまる</span>
      - <span class="comment">月額パスは他のガチャゲームでは非常に一般的な機能であり、ライフワンダーズはこれを真剣に検討する必要がある</span>
  - 有料スキンについても数回言及があった
  - 新規プレイヤーから、ガチャ通貨や育成リソースの蓄積が非常に困難なため、ゲーム序盤が非常に遅くて退屈だという不満があった。
  ...
  - 古いキャラクターのスキルが現在の環境では役に立たないという不満がいくつかある。スキルツリーのより頻繁なリリースが望まれている
      - スキルツリー強化後も古いキャラクターは依然として役に立たないと不満を言う人もいる
  - ある回答者は「アニバーサリーが悪くなった」（具体的な詳細はなし）と述べ、別の回答者はアニバーサリーチケットが何年も3枚のままであることに不満を表明している。

### キャラクター

  - ある回答者は、過去12ヶ月間の星5キャラクターがほとんどマッチョな男性体型ばかりで、ぽっちゃり体型は（ケモノの）1人だけだったと不満を述べた。過去12ヶ月間、ぽっちゃりした人間の限定バージョンはなかった。
  - 女性キャラクターの限定バージョン
  - 全てのキャラクターに限定バージョンが登場する平等な機会
  - 多様性のために、ゲイ男性に人気のないキャラクターの割合を増やすという運営の決定に満足していないプレイヤーもいる
  - 女性プレイヤーは、もっと女性キャラクターが必要だと考えている
  - ショタキャラクターの実装に反対する回答者もいる
      - <span class="comment">これについてはあまりコメントできませんが、東アジアと他の国々では、架空のショタキャラクターに対する見方が大きく異なることは誰もが知っています...</span>
  - もっとケモノキャラクターを
      - もっと女性のケモノキャラクターを
  - もっとおじいさんキャラクターを
  - もっと細身のキャラクターを
...
<span class="comment">ご覧の通り、プレイヤー層が十分に多様化すると、プレイヤーベースから矛盾した意見が出てきます。ライブ・ア・ヒーローが新しいユニット（バージョン違いまたは新キャラクター）をリリースするのがいかに遅いかを考えると、ライフワンダーズが全員を満足させる方法はありません。</span>

<span class="comment">幸いなことに、キャラクタータイプの割合について実際に不満を言う回答者の割合はかなり低いです。コミュニティは一般的に、皆さんが思うよりも寛容です。</span>
  ...
  - 古いキャラクターのSpineモデルの改善
  - 一部の新キャラクターのSpineモデルの品質が他に比べて劣っている。
  - 最近の一部のキャラクターのSpineモデル用アクセサリーが、単純で退屈になっていると不満を言う人もいる
  - Spineモデルのスキンを変更したいという要望がある
  - Spineモデルの股間の膨らみを大きくしてほしい
      - <span class="comment" markdown="1">XXL Woofiaが「SFW」のGoogle Play/Apple Store版で[巨大な股間の膨らみ](https://x.com/ReclamonRex/status/1965350665083039765/)を許されているのなら、ライフワンダーズももう少し大胆になれると思うのですが...</span>
  - HDキャラクターアート
  - キャラクターの足が短すぎる

# バグとパフォーマンス

  - このカテゴリーで最も一般的な不満は、読み込み時間が長すぎること、特にログイン画面での読み込み時間です。
  - ログイン画面でランダムにゲームがクラッシュするという報告も一部あります
  - フレンドリスト管理ページの動作が非常に遅い

## 新規イラストレーターウィッシュリスト

イラストレーターのウィッシュリストに関しては、皆さん本当に遠慮がなかったようで、この質問には膨大な数の名前が寄せられました。

私はイラストレーターを見つけ出すために全力を尽くしましたが、一部の名前は検索が不可能で、中にはタイプミスもありました。

<span class="comment">お気に入りのイラストレーターの名前を間違って入力した方々、ただちにサポーター会員資格を取り消します。会員資格を取り戻すには、その名前を正しく1000回手書きする必要があります。<span>

提案の中には文字通り無謀なものもあるので、ぜひリストをご覧ください。友人のジョークを引用すると、「夢が大きすぎて、（ヒーロー）パスが開ける人もいる」とのことです。
...
実際に声優の希望リストを提供した回答者は、イラストレーターに比べて少ない。大半は声優業界について詳しくないことを認め、LifeWondersの判断を信頼していると述べている。

それでもなお、多くの名前が寄せられている。この質問の処理は予想以上に困難でした。回答者によっては声優名を英語表記（ひらがな）で記入し、アクセント記号付きの場合さえありました。また漢字表記のケースも存在し、さらに複数の誤字や別表記も確認されたため、重複排除が非常に困難でした。

<details open markdown="1">
<summary>複数回答者から言及された声優リスト</summary>
...
<details markdown="1">
<summary>言及回数が少ないその他の声優（順不同）</summary>
...
時間の制約により、回答者が希望したものの実際にこのゲームで既に起用されている声優のリストは作成しませんでした。ただし、{% chara_link Theoreol %}（子安武人）の声優を希望する声がかなり多かったことは特筆すべきでしょう。

# あとがき

本レポートをお楽しみいただければ幸いです。読者の皆様にとって、このレポートを読むのにかかる時間は20分程度（画像だけ見るならさらに短く）かもしれませんが、調査の準備、データの集計、可視化スクリプトのデバッグ、レポート作成の全プロセスは、ほぼ2ヶ月間のフルタイム作業に相当します。また自由回答形式の質問を多用したため、分析にさらに多くの時間を要しました。

公式翻訳の発表がいつになるか分からない間も、このWikiは海外プレイヤーをサポートし続けます。

来年の調査でお会いしましょう！（あるいは、記念日の約2週間前に実施予定の今年の非公式「東京放課後サマナーズ」調査でお会いしましょう。）
